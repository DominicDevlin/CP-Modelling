...
int CellularPotts::DeltaH(int x,int y, int xp, int yp, const int tsteps, PDE *PDEfield)       
{
  ...
  const double lambda2=(*cell)[sxyp].get_lambda_2();
...
void CellularPotts::CellGrowthAndDivision(void) 
{
  if (CountCells() < 300)
  {
    ...
    c++) {
      if (c->AliveP())
      {
        if ( (c->Area()-c->TargetArea())>par.gthresh ) 
        {
        ...
        else if ( (c->Area()-c->TargetArea())<par.shrink ) 
        {
        ...
        else if (c->Area() < 3)
        {
          c->SetTargetArea(0);
          c->set_lambda(100);
          // std::cout << "Cell apoptosing..." << std::endl;
        }   
...
void CellularPotts::set_MF(vector<vector<int>> middles, int gene)
{
  ...
  if (abs(xdif) > abs(ydif))
  {
    if (middles.at(0).at(1) > middles.at(1).at(1))
    {
      ...
      g_list.at(gene) = 1;
      int val = g_list.at(4) * 4 + g_list.at(5)*3;
...
void CellularPotts::start_network(vector<vector<int>> start_matrix)
{
  ...
  if (par.randomise)
    randomise_network();
  else //set matrix to input, either from simulation or par.set_matrix
  {
    for (int i = 0; i < par.n_genes; ++i)
    {
      for (int j = 0; j < par.n_activators; ++j)
  ...
  for ( (c=cell->begin(), c++); c!=cell->end(); c++) 
  {
    if (c->AliveP())
    {
      ...
      for (int i = 0; i < par.n_genes; ++i)
      {
        ...
        else if (i < par.n_genes - par.n_lockandkey/2)
        {
          locks.at(j) = discrete_decay(gene_copy, locks.at(j), i);
          ++j;
        }
        else
        {
          keys.at(k) = discrete_decay(gene_copy, keys.at(k), i);
          ++k;
        }
      }
      ...
      c->set_ctype(set_type(l_bool, k_bool, genes.at(12), genes.at(13)) * c->getTau());
      ...
      if (genes.at(12) > 0.5 && genes.at(13) > 0.5)
      {
        c->SetTargetLength(80.0);
        c->set_lambda_2(par.lambda2);
      }
      else if (genes.at(12) > 0.5 || genes.at(13) > 0.5)
      {
        c->SetTargetLength(50.0);
        c->set_lambda_2(par.lambda2);
      }
      else
      {
...
int CellularPotts::set_type(vector<bool>& lock_b, vector<bool>& key_b, double TL1, double TL2)
{
  vector<bool> new_list{};
  ...
  for (int i = 0; i < par.n_lockandkey + par.n_length_genes; ++i)
  {
    if (i < 5)
    {
      new_list.push_back(lock_b.at(j));
      ++j;
    }
    else if (i < 10)
    {
      new_list.push_back(key_b.at(k));
      ++k;
    }
    else
    {
      new_list.push_back((TL1>0.5) ? true : false);
      new_list.push_back((TL2>0.5) ? true : false);
      ++i;
    }
  }

  // iterate through type list to see if type is already there.
  auto it = find(type_list.begin(), type_list.end(), new_list);
...
int CellularPotts::get_ntypes()
{
  ...
  for ( (c=cell->begin(),c++); c!=cell->end(); c++) 
  {
    if (c->AliveP())
    {
      std::vector<bool> lock_b = c->get_locks_bool();
      std::vector<bool> key_b = c->get_keys_bool();
      vector<bool> new_list{};
      ...
      for (int i = 0; i < par.n_lockandkey + par.n_length_genes; ++i)
      {
        ...
        {
          new_list.push_back((c->get_genes().at(12)>0.5) ? true : false);
          new_list.push_back((c->get_genes().at(13)>0.5) ? true : false);
          ++i;
        }
      }  
      auto it = find(cell_types.begin(), cell_types.end(), new_list);

      if (it == cell_types.end())
      {
        cell_types.push_back(new_list);
      }
    }
  }
  int n_types = static_cast<int>(cell_types.size());
  // cout << "THE FITNESS (N types) is: " << n_types << endl;
