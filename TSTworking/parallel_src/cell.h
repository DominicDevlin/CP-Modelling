...
private:
  ...
  inline double Length(long int s_x,long int s_y,long int s_xx,
    ...
    double rhs2=sqrt( (ixx-iyy)*(ixx-iyy)+4*ixy*ixy )/2;

    if (par.PRINT_OUT == true)
    {
      std::cerr << "ixx: " << ixx << '\n';
      std::cerr << "iyy: " << ixx << '\n';
      std::cerr << "ixy: " << ixx << '\n';
  ...
  inline void set_genes(std::vector<double> new_g)
  {
    genes = new_g;
  }

  inline std::vector<double>& get_genes(void)
  {
    return genes;
  }
  ...
  inline void print_genes(void)
  {
    ...
    for (int i = 0; i < genes.size(); ++i)
    {
      if (i < par.n_diffusers)
        std::cout << " - Gene " << i+1 << " is: " << diff_genes.at(i);
      else
        std::cout << " - Gene " << i+1 << " is: " << genes.at(i);
    }
    std::cout << std::endl << "Neighbour diffusers: ";
    for (int i = 0; i < par.n_diffusers; ++i)
      std::cout << " - Diffuser " << i+1 << " is: " << genes.at(i);
    std::cout << std::endl;
  }


  inline std::vector<double>& get_diffusers(void)
  {
    return diff_genes;
  }

  inline std::vector<double> set_diffusers(std::vector<double> new_diff)
  {
    diff_genes = new_diff;
  }
...
protected:
  ...
  std::vector<double> genes; // should initialise to appropriate value
  //std::map<std::string, double> genes;

  std::vector<double> diff_genes{1.0,1.0,1.0,1.0};
