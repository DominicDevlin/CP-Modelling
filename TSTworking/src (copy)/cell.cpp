...
int Cell::CalculateJfromMed(vector<bool>& key2 )
{
  ...
  Jval += 10; // offset so interaction with medium is not 0
  return Jval;
}
...
int Cell::CalculateJwithMed()
{
  ...
  for (int i = 0; i < par.n_locks; ++i)
  {
    Jval += keys_bool[i] * par.med_table[j];
    ++j;
  }
  Jval +=10;
  return Jval;
...
int Cell::CalculateJfromKeyLock(vector<bool>& key2, vector<bool>& lock2 )
{
  ...
  for (int i =0; i < par.n_locks; ++i)
  {
    score += ( keys_bool[i] != lock2[i] )?1:0;
    score += ( key2[i] != locks_bool[i] )?1:0;
  }
  // perfect score is 10 (all locks and keys match). 
  int J = (int)( 32. - 28. * ((double)score) /(2.*par.n_locks) );
