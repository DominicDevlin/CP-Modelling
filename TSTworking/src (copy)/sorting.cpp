...
INIT {
  ...
  try {
    ...
    for (int i=0;i<par.divisions;i++) {
      std::cout << "NUMBER OF DIVISIONS IS: " << par.divisions << std::endl;
      CPM->DivideCells();
    }
...
TIMESTEP { 
  ...
  try {
    ...
    dish->CPM->AmoebaeMove(i);
    


    // print current number of types
    static vector<int> intra_org_fitness;
    if (i > par.mcs * 0.2 && i % 100)
    {
      intra_org_fitness.push_back(dish->CPM->get_ntypes());
      cout << intra_org_fitness.back() << endl;
    }
    ...
    if (i % 1000 == 0)
    {
      ...
      dish->CPM->get_ntypes();
      cout << i << " TIME STEPS HAVE PASSED." << endl;
    }
